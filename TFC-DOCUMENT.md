...
| Base de données | PostgreSQL | 14+ | SGBD relationnel robuste, adapté aux contraintes transactionnelles et au volume de données du système |
...
| **Chef de Département** | Chef de département | Responsable académique chargé de valider ou rejeter les sujets soumis dans sa filière, d'assigner les directeurs de mémoire et de suivre l'avancement des TFC. |
| **Enseignant** | Enseignant | Directeur de mémoire chargé de superviser les travaux qui lui sont assignés, de consulter les rapports d'analyse IA, d'autoriser la soutenance (Feu Vert) et de retirer cette autorisation si nécessaire avant dépôt de la version finale. |
...
- Recevoir, le cas échéant, la notification de retrait du « Feu Vert » ;
- Consulter la date et la salle de soutenance planifiées ;
...
- Planifier la soutenance (date et salle) après le Feu Vert de l'encadreur ;
...
- Retirer le « Feu Vert » tant que la version finale n'est pas encore déposée.
...
|----|-------------------|-----------------|--------------|---------------|-------------|
| CU01 | Créer un compte utilisateur | Administrateur | Rôle et filière définis | Compte créé et actif | L'administrateur crée le compte utilisateur (nom, e-mail, matricule, rôle, filière) depuis l'interface d'administration. |
...
subjects (id, title, subject_type, description, status, rejection_reason,
          defense_validated, defense_date, defense_room, context_relevance,
          challenges, research_question, hypothesis, general_objective,
          specific_objectives, state_of_art, demarcations, methodologies,
          student_id, teacher_id, department_id, academic_year_id, created_at,
          updated_at)
    PK: id
...
| Subject | defense_date | DATETIME | — | NULLABLE | Date et heure planifiées de soutenance |
| Subject | defense_room | VARCHAR | 100 | NULLABLE | Salle de soutenance |
...
| Laragon / XAMPP + PostgreSQL | Environnement de développement local (PHP, serveur web, SGBD) | — |
...
L'application utilise **22 tables** au total : **10 tables métier** (users, faculties, departments, subjects, thesis_files, ai_reports, academic_years, system_settings, activity_logs, notifications) et **12 tables techniques** (authentification, cache, jobs et permissions RBAC).
...
| defense_date | DATETIME | NULLABLE | Date et heure de soutenance planifiées |
| defense_room | VARCHAR(100) | NULLABLE | Salle de soutenance planifiée |
...
- La **date et la salle de soutenance** lorsqu'elles sont planifiées.
...
- L'**action de planification** (date et salle) pour les sujets ayant reçu le Feu Vert ;
...
- L'**affichage de la planification** (date/salle) lorsqu'elle est définie par le Chef de Département ;
...
|----|-----------------|-------------------|-----------------|--------|
| T01 | Création d'un compte étudiant par l'admin | Compte créé, rôle Étudiant assigné | Compte créé avec succès | ✓ Réussi |
...
| T12 | Planification de soutenance (CP) | `defense_date` + `defense_room` enregistrés (après Feu Vert) | Date/salle visibles sur les dashboards concernés | ✓ Réussi |
| T13 | Consultation des archives (public) | Liste des travaux défendus | Tableau paginé avec filtres fonctionnels | ✓ Réussi |
| T14 | Téléchargement version finale (archives) | Fichier PDF téléchargé | Téléchargement effectué | ✓ Réussi |
| T15 | Accès admin sans rôle Admin | Erreur 403 | Page « Accès non autorisé » | ✓ Réussi |
| T16 | Création d'un utilisateur (Admin) | Utilisateur créé, rôle assigné | Log d'activité enregistré | ✓ Réussi |
| T17 | Blocage d'un utilisateur | is_blocked = true | Connexion impossible pour l'utilisateur | ✓ Réussi |
| T18 | Réinitialisation de mot de passe | Nouveau mot de passe généré | Mot de passe affiché à l'admin | ✓ Réussi |
| T19 | Clôture d'une année académique | is_closed = true | TFC archivés, année verrouillée | ✓ Réussi |
| T20 | Export CSV des sujets | Fichier CSV téléchargé | Format correct avec encodage UTF-8 BOM | ✓ Réussi |
| T21 | Analyse IA avec scores élevés | Alerte visuelle (rouge) | Badge rouge affiché, sections signalées | ✓ Réussi |
| T22 | Retrait du Feu Vert (sans version finale) | defense_validated = false + motif obligatoire + notification de retrait | Retrait effectué, motif enregistré, date/salle effacées | ✓ Réussi |
...
1. **La numérisation complète du processus de gestion des TFC** : depuis la soumission du sujet par l'étudiant jusqu'à la planification de la soutenance et l'archivage du mémoire défendu, chaque étape est désormais dématérialisée et tracée.
...
7. **L'ajout d'un module de planification de soutenance** : après autorisation de l'encadreur, le Chef de Département peut définir la date/heure et la salle, visibles par les acteurs concernés.

## Vérification des hypothèses

- **Hypothèse 1** (plateforme centralisée) : **confirmée**. L'application offre un point d'accès unique à tous les acteurs, avec des tableaux de bord adaptés à chaque rôle et un workflow automatisé allant de la soumission à la planification de la soutenance.
...
- **L'absence de gestion complète des sessions de soutenance** (composition du jury, attribution des notes, procès-verbal) ;
...
- L'extension du **module de soutenance** vers la gestion du jury, la notation et l'édition d'un procès-verbal ;
