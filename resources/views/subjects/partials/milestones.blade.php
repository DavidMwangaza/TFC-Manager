<div class="bg-white shadow-sm sm:rounded-lg p-6">
    ...
    @hasanyrole('Enseignant|Chef de département')
        <div class="mb-6 bg-gray-50 border rounded-lg p-4">
            <h3 class="text-sm font-semibold text-gray-800 mb-3">Nouveau jalon</h3>
            <form action="{{ route('milestones.store', $subject) }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Titre du jalon</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Date limite (Étudiant)</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Délai de correction (SLA Prof)</label>
                ...
                <div class="md:col-span-3 flex justify-end">
                    <button class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white font-medium text-sm rounded-lg hover:bg-indigo-700 shadow-sm transition">
                        <x-icon name="plus-circle" class="w-4 h-4" /> Créer le jalon
                    </button>
                </div>
            </form>
        </div>
                ...
                <div class="relative border {{ $borderClass }} {{ $bgClass }} rounded-xl shadow-sm overflow-hidden transition hover:shadow-md">
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 sm:p-5">
                        ...
                        <div class="shrink-0 flex items-center justify-center w-12 h-12 rounded-full border-2 
                            ...
                            {{ $milestone->status === 'pending' && !$isOverdue ? 'border-gray-300 bg-gray-50 text-gray-500' : '' }}
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2 text-sm text-gray-600">
                                <div class="flex items-center gap-1.5" title="Date limite pour l'étudiant">
                                    ...
                                    <span>Échéance: <strong class="{{ $isOverdue ? 'text-orange-600' : 'text-gray-900' }}">{{ $milestone->due_date?->format('d M Y, H:i') ?? 'Non définie' }}</strong></span>
                                </div>
                                
                                @if($milestone->submission_date)
                                    <div class="flex items-center gap-1.5" title="Date de dépôt du travail">
                                        <x-icon name="arrow-up-on-square" class="w-4 h-4 text-gray-400" />
                                        <span>Soumis le: <strong class="text-gray-900">{{ $milestone->submission_date->format('d M Y, H:i') }}</strong></span>
                                    </div>
                                @endif
                                ...
                                @if($milestone->correction_deadline)
                                    <div class="flex items-center gap-1.5" title="Délai maximum pour la correction du professeur">
                                        ...
                                        <span>SLA Prof: <strong class="text-gray-900">{{ $milestone->correction_deadline->format('d M Y, H:i') }}</strong></span>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        {{-- Actions (Boutons) --}}
                        <div class="shrink-0 flex items-center flex-wrap gap-2 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                                ...
                                @if(in_array($milestone->status, ['pending', 'rejected']))
                                    ...
                                    <x-modal name="milestone-upload-{{ $milestone->id }}" focusable>
                                        <form method="POST" action="{{ route('milestones.upload', $milestone) }}" enctype="multipart/form-data" class="p-6">
                                            @csrf
                                            <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><x-icon name="document-arrow-up" class="w-6 h-6 text-blue-600"/> Dépôt du travail</h3>
                                            <p class="text-sm text-gray-600 mb-4">Vous allez soumettre votre travail pour le jalon : <strong>{{ $milestone->title }}</strong></p>
                                            <div class="mb-5 bg-gray-50 p-4 rounded-lg border border-gray-200">
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Fichier PDF (max 20 Mo)</label>
                                                <input type="file" name="pdf" accept="application/pdf" required class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                                            </div>
                                            <div class="flex justify-end gap-3 mt-6">
                                                <button type="button" @click="$dispatch('close-modal', 'milestone-upload-{{ $milestone->id }}')" class="px-4 py-2 bg-white text-gray-700 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 transition">Annuler</button>
                                ...
                                @if($milestone->status === 'submitted')
                                    ...
                                    <x-modal name="milestone-validate-{{ $milestone->id }}" focusable>
                                        <form method="POST" action="{{ route('milestones.validate', $milestone) }}" class="p-6">
                                            @csrf
                                            <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><x-icon name="check-badge" class="w-6 h-6 text-green-600"/> Validation du jalon</h3>
                                            <p class="text-sm text-gray-600 mb-4">Vous vous apprêtez à valider le jalon : <strong>{{ $milestone->title }}</strong></p>
                                            <div class="mb-5">
                                                <label class="block text-sm font-medium text-gray-700 mb-1">Feedback pour l'étudiant (optionnel)</label>
                                                <textarea name="comments" rows="3" placeholder="Bravo pour cette partie..." class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500"></textarea>
                                            </div>
                                            <div class="flex justify-end gap-3">
                                                <button type="button" @click="$dispatch('close-modal', 'milestone-validate-{{ $milestone->id }}')" class="px-4 py-2 bg-white text-gray-700 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 transition">Annuler</button>
                                                <button type="submit" class="px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition">Valider définitivement</button>
                                            </div>
                                        </form>
                                    </x-modal>
                                    ...
                                    <x-modal name="milestone-reject-{{ $milestone->id }}" focusable>
                                        <form method="POST" action="{{ route('milestones.reject', $milestone) }}" class="p-6">
                                            @csrf
                                            <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><x-icon name="exclamation-triangle" class="w-6 h-6 text-red-600"/> Demande de corrections</h3>
                                            <p class="text-sm text-gray-600 mb-4">Vous refusez le travail actuel pour le jalon : <strong>{{ $milestone->title }}</strong></p>
                                            ...
                                            <div class="flex justify-end gap-3">
                                                <button type="button" @click="$dispatch('close-modal', 'milestone-reject-{{ $milestone->id }}')" class="px-4 py-2 bg-white text-gray-700 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 transition">Annuler</button>
                            ...
                            @if($milestone->thesisFile)
                                <a href="{{ route('thesis.download', $milestone->thesisFile) }}" 
                                   class="flex-1 sm:flex-none inline-flex justify-center items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg border border-gray-200 transition" title="Télécharger le document">
                                    <x-icon name="document-text" class="w-4 h-4 text-gray-500" /> PDF
                                </a>
                            @endif
                        </div>
                    </div>
                    ...
                    @if($milestone->comments)
                        <div class="px-5 py-3 border-t {{ $milestone->status === 'rejected' ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100' }}">
                            <div class="flex gap-3">
                                <div class="shrink-0 mt-0.5">
                                    <x-icon name="chat-bubble-bottom-center-text" class="w-5 h-5 {{ $milestone->status === 'rejected' ? 'text-red-400' : 'text-gray-400' }}" />
                                </div>
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wider {{ $milestone->status === 'rejected' ? 'text-red-800' : 'text-gray-500' }} mb-1">
                                        Feedback du Professeur
                                    </p>
                                    <p class="text-sm {{ $milestone->status === 'rejected' ? 'text-red-700 font-medium' : 'text-gray-700' }}">
                                        {{ $milestone->comments }}
                                    </p>
                                </div>
                            </div>
    ...
    @else
        <div class="flex flex-col items-center justify-center py-10 px-4 text-center bg-gray-50 rounded-xl border border-dashed border-gray-300">
            <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm border border-gray-100 mb-4">
                <x-icon name="flag" class="w-8 h-8 text-gray-300" />
            </div>
            <h3 class="text-base font-semibold text-gray-900 mb-1">Aucun jalon défini</h3>
            <p class="text-sm text-gray-500 max-w-sm">Le suivi de ce TFC n'a pas encore été découpé en étapes. Les jalons apparaîtront ici une fois créés.</p>
        </div>
    @endif
</div>
