<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight inline-flex items-center gap-2">
            <x-icon name="document-text" class="w-6 h-6" /> Fiche de Proposition de Sujet
        </h2>
    </x-slot>
            ...
            <div x-data="{
                        ...
                        <template x-for="s in totalSteps" :key="s">
                            <div class="flex items-center" :class="s < totalSteps ? 'flex-1' : ''">
                                <div class="flex items-center justify-center w-10 h-10 rounded-full text-sm font-bold transition-all duration-300"
                                    :class="step >= s ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500'">
                                    <span x-text="s"></span>
                                </div>
                                <div x-show="s < totalSteps" class="flex-1 h-1 mx-2 rounded transition-all duration-300"
                                    :class="step > s ? 'bg-blue-600' : 'bg-gray-200'"></div>
                            </div>
                        </template>
                    </div>
                ...
                <form method="POST" action="{{ route('subjects.store') }}" id="subjectForm" @submit="submitting = true">
                    ...
                    {{-- ═══════════════════════════════════════════════
                         ...
                         ═══════════════════════════════════════════════ --}}
                    <div x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="bg-white shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">Informations Générales</h3>
                        <p class="text-sm text-gray-500 mb-6">Ces informations sont extraites de votre profil connecté.</p>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-500">Nom et Prénom</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-500">Filière</label>
                                <div class="mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md text-gray-800 font-medium">
                    ...
                    {{-- ═══════════════════════════════════════════════
                         ...
                         ═══════════════════════════════════════════════ --}}
                    <div x-show="step === 2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="bg-white shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">Titre du Projet</h3>
                        <p class="text-sm text-gray-500 mb-6">Choisissez un titre clair et descriptif pour votre travail.</p>

                        <div class="space-y-6">
                            <div>
                                <label for="title" class="block text-sm font-medium text-gray-700">Titre du sujet <span class="text-red-500">*</span></label>
                                ...
                                <p class="mt-1 text-xs text-gray-500">Maximum 255 caractères — Soyez précis et descriptif.</p>
                            </div>

                            <div>
                                <label for="subject_type" class="block text-sm font-medium text-gray-700">Type de sujet</label>
                                ...
                                @error('subject_type') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- ═══════════════════════════════════════════════
                         ...
                         ═══════════════════════════════════════════════ --}}
                    <div x-show="step === 3" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="bg-white shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">Construction du Problème</h3>
                        <p class="text-sm text-gray-500 mb-6">Structurez votre problématique en 3 parties distinctes.</p>

                        <div class="space-y-6">
                            {{-- A: Contexte et Pertinence --}}
                            <div>
                                <label for="context_relevance" class="block text-sm font-medium text-gray-700">
                                    A. Contexte et Pertinence <span class="text-red-500">*</span>
                                </label>
                                <p class="text-xs text-gray-500 mb-1">Expliquez pourquoi ce sujet est important maintenant. Quel est le contexte ?</p>
                                ...
                                @error('context_relevance') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.context_relevance" x-text="errors.context_relevance" class="text-red-500 text-xs mt-1"></p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-1">Qu'est-ce qui ne fonctionne pas actuellement ? Quels sont les problèmes identifiés ?</p>
                                ...
                                @error('challenges') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.challenges" x-text="errors.challenges" class="text-red-500 text-xs mt-1"></p>
                            </div>

                            {{-- C: Question de Recherche --}}
                            <div>
                                <label for="research_question" class="block text-sm font-medium text-gray-700">
                                    C. Question de Recherche <span class="text-red-500">*</span>
                                </label>
                                <p class="text-xs text-gray-500 mb-1">Posez votre question de recherche clairement.</p>
                                ...
                                @error('research_question') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.research_question" x-text="errors.research_question" class="text-red-500 text-xs mt-1"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- ═══════════════════════════════════════════════
                         ...
                         ═══════════════════════════════════════════════ --}}
                    <div x-show="step === 4" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="bg-white shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">Solution et Objectifs</h3>
                        <p class="text-sm text-gray-500 mb-6">Décrivez votre solution envisagée et définissez vos objectifs.</p>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-1">Quelle est votre solution provisoire au problème posé ?</p>
                                ...
                                @error('hypothesis') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.hypothesis" x-text="errors.hypothesis" class="text-red-500 text-xs mt-1"></p>
                            </div>

                            {{-- Objectif Général --}}
                            <div>
                                <label for="general_objective" class="block text-sm font-medium text-gray-700">
                                    Objectif Général <span class="text-red-500">*</span>
                                </label>
                                <p class="text-xs text-gray-500 mb-1">Le but final de votre projet en une phrase.</p>
                                ...
                                @error('general_objective') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.general_objective" x-text="errors.general_objective" class="text-red-500 text-xs mt-1"></p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-2">Listez vos objectifs spécifiques. Cliquez sur « + » pour en ajouter.</p>

                                <div class="space-y-2">
                                    <template x-for="(obj, index) in specificObjectives" :key="index">
                                        <div class="flex items-center gap-2">
                                            <span class="text-sm text-gray-400 w-6 text-right" x-text="(index+1) + '.'"></span>
                                ...
                                <button type="button" @click="addObjective()"
                                    class="mt-2 inline-flex items-center px-3 py-1.5 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition">
                                    + Ajouter un objectif
                                </button>
                                @error('specific_objectives') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                @error('specific_objectives.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                <p x-show="errors.specific_objectives" x-text="errors.specific_objectives" class="text-red-500 text-xs mt-1"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- ═══════════════════════════════════════════════
                         ...
                         ═══════════════════════════════════════════════ --}}
                    <div x-show="step === 5" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="bg-white shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-1">Cadre Scientifique et Méthodologique</h3>
                        <p class="text-sm text-gray-500 mb-6">Présentez votre état de l'art, vos démarcations et vos méthodes.</p>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-2">Remplissez le tableau avec les travaux existants liés à votre sujet.</p>
                                    ...
                                    <table class="w-full text-sm">
                                        <thead>
                                            <tr class="bg-gray-50">
                                                ...
                                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Auteur</th>
                                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Institution</th>
                                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Apport / Sujet</th>
                            ...
                            {{-- Démarcations --}}
                            <div>
                                <label for="demarcations" class="block text-sm font-medium text-gray-700">
                                    Démarcations
                                </label>
                                <p class="text-xs text-gray-500 mb-1">En quoi votre travail se distingue-t-il des travaux existants ?</p>
                                ...
                                @error('demarcations') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>

                            {{-- Méthodologies --}}
                            <div>
                                <label for="methodologies" class="block text-sm font-medium text-gray-700">
                                    Méthodologies
                                </label>
                                <p class="text-xs text-gray-500 mb-1">Décrivez vos méthodes de collecte et d'analyse (ex: Interview, UML, UP, Merise...).</p>
                                ...
                                @error('methodologies') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div>
                                ...
                                <a x-show="step === 1" href="{{ route('subjects.index') }}" class="text-sm text-gray-600 hover:text-gray-900 hover:underline">
                                    ← Retour à la liste
                                </a>
                            </div>
                            ...
                            <div>
                                <button type="button" x-show="step < totalSteps" @click="nextStep()"
                                    class="inline-flex items-center px-6 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-sm text-white hover:bg-blue-700 transition">
                                    Suivant →
                                </button>
                                <button type="submit" x-show="step === totalSteps" x-cloak :disabled="submitting"
                                    ...
                                    <span x-show="!submitting">Soumettre la proposition</span>
